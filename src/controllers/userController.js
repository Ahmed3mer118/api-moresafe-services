  ...
  try {
    ...
    res.json(
      users.map((u) => {
        ...
        safe.projects = (safe.projects || []).map(normalizeUserProject).filter(Boolean);
        const seen = new Set(safe.projects.map((p) => p.id));

        for (const p of roleProjects) {
          const isLinked =
            String(p.manager) === String(u._id) ||
            p.accountants.some((a) => String(a) === String(u._id));
          if (isLinked && !seen.has(String(p._id))) {
            seen.add(String(p._id));
            const normalized = normalizeUserProject(p);
            if (normalized) safe.projects.push(normalized);
          }
        }
